...
class AccountInvoice(models.Model):
    ...
    refund_invoice_id = fields.Many2one('account.tax.invoice', string="Invoice for which this invoice is the credit note")
    ...
    vendor_bill_id = fields.Many2one('account.tax.invoice', string='Vendor Bill',
    ...
    invoice_line_ids = fields.One2many('account.tax.invoice.line', 'invoice_id', string='Invoice Lines', oldname='invoice_line',
        readonly=True, states={'draft': [('readonly', False)]}, copy=True)
    tax_line_ids = fields.One2many('account.tax.invoice.tax', 'invoice_id', string='Tax Lines', oldname='tax_line',
        readonly=True, states={'draft': [('readonly', False)]}, copy=True)
    refund_invoice_ids = fields.One2many('account.tax.invoice', 'refund_invoice_id', string='Refund Invoices', readonly=True)
    ...
    company_id = fields.Many2one('res.company', string='Company', change_default=True,
        required=True, readonly=True, states={'draft': [('readonly', False)]},
        default=lambda self: self.env['res.company']._company_default_get('account.tax.invoice'))
    ...
    def _onchange_vendor_bill(self):
        ...
        new_lines = self.env['account.tax.invoice.line']
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        supplier_form_view_id = get_view_id('invoice_supplier_form', 'account.tax.invoice.supplier.form').id
        if context.get('active_model') == 'res.partner' and context.get('active_ids'):
            partner = self.env['res.partner'].browse(context['active_ids'])[0]
            if not view_type:
                view_id = get_view_id('invoice_tree', 'account.tax.invoice.tree')
                view_type = 'tree'
            elif view_type == 'form':
                if partner.supplier and not partner.customer:
                    view_id = supplier_form_view_id
                elif partner.customer and not partner.supplier:
                    view_id = get_view_id('invoice_form', 'account.tax.invoice.form').id
    ...
    #     ctx = dict(
    #         default_model='account.tax.invoice',
    ...
    #         'view_type': 'form',
    #         'view_mode': 'form',
    #         'res_model': 'account.tax.invoice.send',
    ...
    def compute_taxes(self):
        ...
        account_invoice_tax = self.env['account.tax.invoice.tax']
        ...
        if self.cash_rounding_id and self.type in ('out_invoice', 'out_refund'):
            ...
            if not self.currency_id.is_zero(rounding_amount):
                if self.cash_rounding_id.strategy == 'biggest_tax':
                    ...
                    rounding_line = self.env['account.tax.invoice.line'].new({
                    ...
                    # ensure the inverse field is updated on account.tax.invoice.
    ...
    def get_formview_id(self, access_uid=None):
        ...
        if self.type in ('in_invoice', 'in_refund'):
            return self.env.ref('aop_sale.tax_invoice_supplier_form').id
        else:
            return self.env.ref('aop_sale.tax_invoice_form').id

    def _prepare_tax_line_vals(self, line, tax):
        """ Prepare values to create an account.tax.invoice.tax line

        The line parameter is an account.tax.invoice.line, and the
    ...
    def refund(self, date_invoice=None, date=None, description=None, journal_id=None):
        ...
        for invoice in self:
            ...
            if invoice.type == 'out_invoice':
                message = _("This customer invoice credit note has been created from: <a href=# data-oe-model=account.tax.invoice data-oe-id=%d>%s</a><br>Reason: %s") % (invoice.id, invoice.number, description)
            else:
                message = _("This vendor bill credit note has been created from: <a href=# data-oe-model=account.tax.invoice data-oe-id=%d>%s</a><br>Reason: %s") % (invoice.id, invoice.number, description)
...
class AccountInvoiceLine(models.Model):
    ...
    def _set_additional_fields(self, invoice):
        """ Some modules, such as Purchase, provide a feature to add automatically pre-filled
            ...
            :param invoice : account.tax.invoice corresponding record
            :rtype line : account.tax.invoice.line record
        """
        pass
...
class AccountInvoiceTax(models.Model):
    ...
    invoice_id = fields.Many2one('account.tax.invoice', string='Invoice', ondelete='cascade', index=True)
...
class AccountPaymentTerm(models.Model):
    ...
    @api.multi
    def unlink(self):
        if self.env['account.tax.invoice'].search([('payment_term_id', 'in', self.ids)]):
